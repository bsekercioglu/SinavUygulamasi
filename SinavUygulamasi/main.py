...
async def sinav_suresi(dakika):
    print("Sınav süresi {} dakikadır.".format(dakika))

    for d in range(dakika * 60, 0, -1):
        await asyncio.sleep(1)
        if d % 60 == 0:
            print(f"{d // 60} dakika kaldı.")
        elif d == 30:
            print("Sınavın bitmesine son 30 saniye kaldı.")
    print("Sınav süresi doldu. Sınav sonlandırılıyor.")

async def sinav_uygulamasi():
...
async def main():
    global sinav_suresi_task
    sinav_suresi_task = asyncio.create_task(sinav_suresi(1))
    sinav_uygulamasi_task = asyncio.create_task(sinav_uygulamasi())

    await sinav_uygulamasi_task

    sinav_suresi_task.cancel()
    try:
        await sinav_suresi_task
    except asyncio.CancelledError:
        print("Sınav süresi doldu. Sınav sonlandırıldı.")
